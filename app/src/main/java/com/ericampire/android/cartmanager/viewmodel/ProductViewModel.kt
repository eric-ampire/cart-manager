...
class ProductViewModel : ViewModel(), KoinComponent {
    ...
    private val categoryProductDao by inject<CategoryProductDao>()
    ...
    val categoryProducts: LiveData<List<CategoryProduct>>
        get() = categoryProductDao.getAllProducts().asLiveData()
    ...
    fun addCategoryProduct(product: CategoryProduct) {
        viewModelScope.launch {
            categoryProductDao.insert(product)
        }
    }
    ...
    fun addCategpryProductToCart(product: CategoryProduct) {
        viewModelScope.launch {
            val cartItem = CartItem(productId = product.id!!, productQuantity = 1)
            cartItemDao.insert(cartItem)
        }
